                                                                ...
                                                                int ledCount, int offset, CorsairDeviceUpdateQueue updateQueue)
    {
        return deviceType switch
        {
            ...
            CorsairChannelDeviceType.EightLedSeriesFan =>
                new CorsairFanRGBDevice(new CorsairFanRGBDeviceInfo(device, ledCount, offset, device.model == "iCUE LINK System Hub" ? "RX Fan" : "8-Led-Series Fan"), updateQueue),
            ...
            _ => CreateUnknownChannelDevice(device, ledCount, offset, updateQueue)
        };
    }

    private static ICorsairRGBDevice CreateUnknownChannelDevice(_CorsairDeviceInfo device, int ledCount, int offset, CorsairDeviceUpdateQueue updateQueue)
    {
        ...
        if ((device.model == "iCUE LINK System Hub") && (ledCount == 18))
            return new CorsairFanRGBDevice(new CorsairFanRGBDeviceInfo(device, ledCount, offset, "LX Fan"), updateQueue);
